...
def main() -> int:
    ...
    for page in html_pages:
        ...
        if path == "/published-paper-reviews.html":
            for marker in ("20 papers", "eleven coauthored published papers", "one accepted coauthored manuscript"):
                if marker not in text:
                    failures.append(f"{path}: missing public archive marker {marker!r}")
            if text.count('class="paper card"') != 20:
                failures.append(f"{path}: expected 20 reviewed-work cards")
            if text.count('class="paper-disclosure"') != 12:
                failures.append(f"{path}: expected 12 per-work coauthor disclosures")
            ...
            expected_review_codes = {"P01", "P02", "P04", "P05", "P06", "P07", "P08", "P10", "P11", "P13", "P15", "P16", "P17", "P18", "P19", "P20", "P21", "P22", "P23", "A01"}
            actual_review_codes = set(re.findall(r'data-code="([PA]\d{2})"', text))
            if actual_review_codes != expected_review_codes:
                failures.append(
                    f"{path}: reviewed-work allowlist mismatch: "
                    f"expected {sorted(expected_review_codes)}, found {sorted(actual_review_codes)}"
                )
            for forbidden in ("public draft", "Before making the archive public", "Coauthored papers are omitted."):
